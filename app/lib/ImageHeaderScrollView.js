// Borrowed from https://github.com/bamlab/react-native-image-header-scroll-view
// To try call listeners without janky <TriggerView>
// Used as long as I find something better (or the package fixes some issues)
...
class ImageHeaderScrollView extends Component {
  ...
  componentDidUpdate() {
    if (this.headerHeight.__getValue() === this.props.minHeight && !this._isMin) {
      this.props.onMinIn()
      this._isMin = true
    } if (this.headerHeight.__getValue() > this.props.minHeight && this._isMin) {
      this.props.onMinOut()
  ...
  render() {
    ...
    return (
      <View
        ...
        onLayout={(x, y) => this.setState({ pageY: y })}
      >
        <ScrollView
